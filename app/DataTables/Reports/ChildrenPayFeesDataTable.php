...
class ChildrenPayFeesDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('name', function (PayFees $model) {
    ...
    public function query(PayFees $model)
    {
        ...
        if (Auth::user()->kindergarten_id != null) {
            $query = $model->whereHas('Children' , function($query){
        ...
        if (!empty($children)) {
            $query =  $query->where('children_id', $children);
        }
        if (!empty($year)) {
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::computed('name')->title('الاسم')->addClass('text-center'),
